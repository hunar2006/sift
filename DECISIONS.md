...
- The directive detector also treats Go's canonical `// Deprecated:` doc comments and `@deprecated` / `@internal` tags in JSDoc blocks across JavaScript and TypeScript as `LINT_SUPPRESSED`. The CLI version remains `0.5.0`; this is another pre-launch correction folded into that initial artifact.

## 2026-07-13 — v0.6 Refinery

- Version is now `0.6.0` in both the published CLI manifest and the surfaced core `SIFT_VERSION`; the release runbook tag is `v0.6.0`. This is the first new pre-launch artifact after the v0.5.3 corrective carry-over.
- The health gate adds only the permitted development dependencies (`knip`, `madge`). Knip is clean after removing unused root test/format/UI dependencies and unused package dependencies; its small config exclusions are documented for public API exports, build tooling, dynamic preflight resolution, and CLI parser-provided runtime modules. Madge reports **0 circular dependencies** after type-only coverage/rule contracts were extracted.
- Bundle debt: the prior universal Shiki build produced an 804.7 kB raw Emacs Lisp grammar and Vite's large-chunk warning. Shiki now loads a small runtime plus per-language grammars; the largest remaining lazy grammar is C++ at 709.4 kB raw / 52.7 kB gzip, Vite emits no warning, and the enforced bundle checker measures the largest initial JavaScript asset (the Shiki runtime) at 55.6 kB gzip, well under the 350 kB budget.
- Type review: `noUncheckedIndexedAccess` was already enabled. `exactOptionalPropertyTypes` was evaluated but would require 107 mechanical changes (core 50, adapter 2, CLI 29, web 23, eval 3), so it remains off under the under-30-fix policy.
- `pnpm test:soak` completed **10/10** full-suite passes (291 tests each); final aggregate line coverage was 84.66%. The 400-file / roughly-25k-line performance median is **878.7 ms** with 7.7 ms review JSON serialization, effectively unchanged from the 879.1 ms pre-word-diff measurement.
- Sweep: the six TODO/FIXME matches are retained classifier/history/test/fuzz fixtures; there are no production TODO/FIXME comments. All console uses are sanctioned CLI/script diagnostics or test literals, and no executable `debugger` was found. Error-path text was spot-checked for recovery wording and blank views. `pnpm audit` has one low development-only esbuild advisory (`GHSA-g7r4-m6w7-qqqr`) through `tsup`; no dependency upgrade was taken during release hardening.
- Craft log v0.6, loop 1: inspected the workbench, queue, and inspector captures; the copy-rule controls were visually too quiet, so they received a visible outline while retaining the quiet palette. Loop 2: added deterministic search and report captures; this exposed Windows shell-splitting of `node.exe` in the shots runner, fixed by bypassing the shell for executable paths. The recaptured dark/light workbench, queue, inspector, search, report, focus, timeline, and completion assets were visually inspected. Queue path rows now de-duplicate later same-file entries, file paths truncate with a tooltip, scrollbars match the theme, nits/sort/theme/split persist, and toast messages retain the newest three.
- Word-level diffing is an in-house, deterministic LCS pass with 400-changed-line, 1,000-character-line, and 20% overlap guards. It is serialized lazily by the API, bold in the terminal cockpit, stronger-but-not-bold in the web view, and rendered in static HTML reports. The smoke proof finds segments on at least five demo hunks.
- Search is intentionally literal and case-insensitive across diff text, paths, and digest text; it is lazy client-side state, supports Ctrl/Cmd+F, palette access, cycling, minimap ticks, and reduced-motion-safe highlighting. TUI search remains out of scope for this release.
...
- pnpm 11 forwards a literal nested separator to Commander as an argument. The root shortcuts use pnpm's equivalent working argument form instead.
- Final full `pnpm preflight` is **PASS**: Stage A passed all eight gates (311 tests, 84.9% lines, 859.2 ms median); eval/fuzz, fresh-user, installed-package, and audit stages passed; Stage C confirmed v0.6.1; and the manual recording stage remains the documented `SKIP`. Neither v0.6.1 cut-line was taken.

## 2026-07-14 - v0.8 Sightline

- Decisions are journaled locally in `.sift/journal.jsonl` after the state write, with JSONL tolerance for an interrupted malformed line and atomic pruning to the latest 500 entries. Status changes, group approval, undo, redo, and targeted undo record their origin; a group approval shares one compound id.
- The original focus regression is fixed at the contract level: status decisions return to the diff pane, while the note textarea is focused only through `i` or an explicit click. The diff has a quiet focus cue, decision restore pulses it, and modal focus is trapped.
- Queue membership is now checked against the final band/category result. Development and test builds throw on a mismatch; production logs and deterministically reassigns. The score-5 Skim regression is covered.
- Sightline ships Graphite (default), Assay, and Paper with local Geist and JetBrains Mono assets. Code size cycles between 12, 13, and 14px and theme/filter preferences persist locally.
- Cut: Sift had no existing file-revert write path, so the decision journal does not introduce one. `R`/file-byte revert remains intentionally unavailable rather than creating a second mutation pathway; targeted undo safely covers status decisions only.

## 2026-07-16 - v0.9 Ground Truth: audit root causes recorded before remediation

The v0.8 automated gate was green (342 tests), but it did not prove the rendered interaction loop. The findings below are code-traced root causes recorded before their remediation; the named v0.9 regression tests are the acceptance evidence.

- **BUG-01 â€” undo/redo key semantics:** `keyboardCommand` mapped raw `Z` to redo without inspecting `shiftKey`; browser shifted-key delivery and the documented `Z` undo alias therefore disagreed. `BUG-01-undo-redo-shift-z-round-trips-counter` locks the browser-level contract.
- **BUG-02 â€” queue and header could disagree after a decision:** `App.updateStatus` updated the local session, persisted, then called `refreshStatsOnly`; server data could replace the optimistic model mid-decision, while the queue, selected hunk, and counters were calculated by separate paths. `BUG-02-store-first-live-decision-selectors` covers a no-reload decision sequence.
- **BUG-03 â€” persistence failure silently removed the human decision:** the `catch` in `App.updateStatus` restored the prior status and emitted only a generic retry toast. It neither retained a retry payload nor marked unsaved decisions. `BUG-03-unsaved-decision-retries-without-state-loss` covers fail, retry-success, and repeat failure.
- **BUG-04 â€” flagged-only could become a blank/stale view:** `visible` and the independently filtered queue used different derived lists; `selected` could point to an item absent from the filtered list, and there was no intentional zero-flag state. `BUG-04-flagged-filter-empty-state-and-live-rows` covers live flagging and the empty state.
- **BUG-05 â€” reviewed counts were not one selector:** the header consumed server `stats.reviewedReviewableLines`, group rows counted the locally rendered subset, and `refreshStatsOnly` replaced only part of that data. `BUG-05-counter-invariant-after-mixed-decisions` makes the source and DOM sums agree.
- **BUG-06 â€” overlays had independent booleans:** palette, help, search, timeline, modal, and focus mode were each controlled separately; the global handler returned early for only two modal types, so a layer could be visible yet keyboard-inert. `BUG-06-overlay-stack-audit-repro` covers the exact audited sequence and palette-over-focus round trip.
- **BUG-07 â€” shifted commands were not consistently dispatched:** the command switch used raw case for some commands but had no explicit modifier policy, leaving `f/F`, `z/Z`, and `R` inconsistent across browser and test delivery. `BUG-07-shifted-key-matrix-and-filter-persistence` is the matrix regression.
- **BUG-08 â€” general notifications never expired:** only `decisionToast` had a timer; the capped `toastStack` had no dismissal lifecycle. `BUG-08-toast-stack-lifecycle` fires five notices and verifies cap, pause, and expiry.
- **BUG-09 â€” history described status strings, not decisions:** journal `action` was mechanically `${from}→${to}` and the panel inferred labels from the target status, so undoing approval was indistinguishable from unflagging. `BUG-09-journal-transition-verbs-and-stale-refusal` asserts transition verbs and stale targeted-undo refusal.
- **BUG-10 â€” syntax painting was best-effort and could fall back silently:** the asynchronous highlighter accepted a failure/fallback result without a rendered-token assertion, so production could render plain text while unit code remained green. `BUG-10-syntax-token-colour-per-theme` requires a rendered token colour distinct from base code text in every theme.
- **BUG-11 â€” help exposed controls with no state change:** the category-looking help controls were present without a functional section filter. They are removed under the explicit v0.9 cut-line; `BUG-11-help-has-no-inert-category-controls` guards that honesty.
- **BUG-12 â€” coverage was formatted with two incompatible units:** the header printed `coverageOnChangedLines.toFixed(0)` while completion multiplied the same fraction by 100. `BUG-12-coverage-format-is-shared` asserts one formatter for both surfaces.
- **BUG-13 â€” inspector actions were below variable-length evidence:** the review controls were rendered in normal inspector flow after the risk/reason sections, so reason-heavy hunks pushed them out of view. `BUG-13-review-actions-sticky-below-risk` asserts the pinned action strip.
- **BUG-14 â€” explicit refresh and copy lacked immediate truthful feedback:** successful `refresh()` replaced data but emitted no success toast; copy waited for both report fetch and clipboard completion before confirming. `BUG-14-refresh-and-copy-feedback` asserts their visible feedback paths.
- **BUG-15 â€” release controls were only partially real:** theme and key-map claims were tested through store helpers rather than the rendered select/keyboard loop, so selecting a theme, invoking a documented command, or seeing its palette was not a UI contract. `BUG-15-theme-dropdown-and-honest-keymap` exercises the live select and published command list.

The separate v0.7 revert omission was structural: there was no snapshot-first mutation boundary at all, and v0.8 explicitly cut the feature. v0.9 introduces one narrowly allowlisted core boundary (`revert.ts`) with byte-exact restoration tests rather than extending ad-hoc state undo.
...
## 2026-07-19 - v1.0 Hallmark: the assay bench (redesign/kimi)

Full frontend redesign on branch `redesign/kimi`. Scope: `packages/web/**`, design tokens, Shiki themes, `scripts/shots.ts` timing, screenshots, README image rows. No keymap, API, CLI, or core behavior changes; every v0.9 DOM contract selector retained, and the e2e suite passes unmodified.

- **Concept.** Sift weighs machine output; the human punches the hallmark. Three genuinely different rooms replace the two near-identical dark themes: **Graphite** (night bench: neutral charcoal `#0D0F12/#13161A/#1A1E24`, phosphor-mint verdict `#2FD6A8`), **Assay** (the office: ink-blue `#070C15/#0C1422/#131E30`, teal `#35E0C0`, brass geometry `#B08D4F`), **Paper** (the certificate: warm stock `#F3F0E8/#FBF9F4/#ECE8DC`, stamp-pad teal `#0B7E68`, umber geometry `#8A6D2F`).
- **Color law.** `--verdict` teal owns approval/progress only; severity is a graded spectrum (critical `#E85A52`, high, medium, low) and never wallpaper; `--brass` is structural geometry only and carries no meaning; `--add`/`--del` are dedicated diff hues distinct from verdict.
- **Type.** Geist out, Space Grotesk Variable in (Fontsource-local, wght axis); JetBrains Mono Variable stays for code and tabular data. Eyebrows 10px/600/+0.11em, digest headlines 16px/650, risk lockup 34px mono/650.
- **Diff legibility.** The faint-wash complaint is resolved structurally: wash at 10.5-13% plus a solid 3px gutter bar plus tinted line-number cells (three cues instead of one faint one), word-change marks at 21-27% with 2px radius. The selected hunk band tints the diff header top hairline; the minimap is a survey rail with widened severity ticks and a bright current marker.
- **Instrument details.** Inspector gains a severity spectrum gauge under the risk lockup (band-colored needle at the score); queue group headers gain a non-truncating mono tally and a brass ledger rule; corner registration marks (2px brass ticks) frame focus cards, modals, the decision log, and the completion certificate; the completion plate becomes a framed certificate with stat dividers and a quieter sieve watermark.
- **Motion.** One vocabulary (snap/settle/glide springs). The orchestrated moment is redesigned as a double-ring hallmark seal that punches from 1.24x with a fading shock ring, held 640ms so it can actually be read. `prefers-reduced-motion` collapses all of it.
- **Root-cause fix: the stamp was invisible.** `.shell { isolation: isolate }` (v0.4.1) created a stacking context that trapped the stamp overlay below the Radix focus dialog (opaque card painted over it). The stamp overlay now renders through `createPortal` to `document.body` (z-70 above portal layers). Verified live: `docs/screenshots/stamp.png` (VERIFIED) and the FLAGGED solid-red variant in `.demo` evidence captures.
- **Flagged checkpoint.** Was full-viewport top-aligned sprawl; now a centered headline, plated concern rows, and a centered primary action (`docs/screenshots/flagged-checkpoint.png`).
- **Self-critique loops.** Loop 1 (first shots): add-wash slab obscured code, group tally truncated by ledger ticks, minimap near-invisible, completion watermark blobby, focus shot captured mid-animation (shots.ts now waits 450ms). Loop 2 (reshoot + evidence captures): stamp missing entirely (root cause above), assay brass too shy (ledger rules + corner ticks), checkpoint sprawl. Both loops fixed and re-captured.
- **Gates.** `pnpm check` green (410 unit tests incl. contrast suite strengthened 86 to 113 checks: new `--brass`/`--add`/`--del`/`--subtle` 3:1 pairs and `--on-verdict` on `--verdict` at 4.5:1 across all three themes), `pnpm e2e` 22/22 with zero selector edits, bundle budget 154.3 kB gzip across 3 initial chunks (largest 98.0 kB, shiki still lazy). Fonts remain local woff2; no network requests.
