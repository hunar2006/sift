...
- The forbidden-verdict guardrail lives in `digest.ts` (`FORBIDDEN_VERDICT_PATTERNS`) and is enforced three ways: a unit test scans every headline template and every signal phrase, and the smoke script scans the live demo model's digests. Forbidden strings are `looks good`, `safe to approve`, `ready to approve`, and `lgtm`.
